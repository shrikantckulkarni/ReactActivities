...
public class BaseReactActivityValidator<T, TDto> : AbstractValidator<T> where TDto : ReactActivityBase
{
    public BaseReactActivityValidator(Func<T,TDto> selector)
    {
         RuleFor(x => selector(x).Title).NotEmpty().WithMessage("Title is required")
        .MaximumLength(100).WithMessage("Title must not exceed length 100");
